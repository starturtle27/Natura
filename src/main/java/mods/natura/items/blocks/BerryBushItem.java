...
public class BerryBushItem extends MultiItemBlock {
    ...
    public static final String blockType[] = { "rasp", "blue", "black", "geo", "rasp", "blue", "black", "geo", "rasp",
            "blue", "black", "geo", "rasp", "blue", "black", "geo" };
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (side != 1) return false;
            ...
            if (block != null && block.canSustainPlant(world, x, y, z, ForgeDirection.UP, NContent.berryBush)
                    && world.isAirBlock(x, y + 1, z)) {
                world.setBlock(x, y + 1, z, NContent.berryBush, stack.getItemDamage() % 4, 3);
                if (!player.capabilities.isCreativeMode) stack.stackSize--;
                if (!world.isRemote) world.playAuxSFX(2001, x, y, z, Block.getIdFromBlock(NContent.berryBush));
                return true;
            } else return false;
        } else return false;
    }
     ...
     * StringBuilder()).append("block.").append(blockType[itemstack.getItemDamage()]).append("berryBush").toString(); }
     */
