...
public class BowlStew extends ItemFood {
    ...
    public IIcon getIcon(ItemStack stack, int renderPass) {
        if (renderPass == 0) {
            ...
            if (stackDamage == 0) return Items.bowl.getIconFromDamage(0);
            return NContent.bowlEmpty.getIconFromDamage(stack.getItemDamage() % 14 - 1);
        } else return icons[stack.getItemDamage() / 14];
    }
    ...
    public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (!player.capabilities.isCreativeMode) {
            ...
            if (stack.stackSize <= 0) {
                int stackDamage = stack.getItemDamage() % 14;
                if (stackDamage == 0) return new ItemStack(Items.bowl);
            ...
            int stackDamage = stack.getItemDamage() % 14;
            if (stackDamage != 0) returnStack = new ItemStack(NContent.bowlEmpty, 1, stackDamage - 1);
