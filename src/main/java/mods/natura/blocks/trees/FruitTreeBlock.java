...
public class FruitTreeBlock extends BlockLog {
    ...
    /*
     * public int getIcon(int side, int metadata) { int tex = blockIndexInTexture + (metadata % 4); int orientation =
     * metadata / 4; switch (orientation) //Ends of logs { case 0: if (side == 0 || side == 1) return tex + 16; break;
     * case 1: if (side == 4 || side == 5) return tex + 16; break; case 2: if (side == 2 || side == 3) return tex + 16;
     * break; } return tex; }
     */
     ...
     * @Override public void getSubBlocks(int par1, CreativeTabs par2CreativeTabs, List par3List) { for (int i = 0; i <
     * 4; i++) par3List.add(new ItemStack(par1, 1, i)); }
     */
}
